...
* Mở ngoặc nhọn của Class và Method phải nằm ở dòng tiếp theo, và đóng ngoặc nhọn tại dòng cuối phía sau nội dung.
* Phạm vi (public, protected, private) cần được khai báo trong toàn bộ property và method; `abstract` và `final` phải khai báo trước phạm vi; còn `static` thì phải khai báo phía sau phạm vi.
* Từ khóa của cấu trúc điều khiển (if, else, for, foreach, while, switch, ...) phải có một whitespace phía sau, tuy nhiên việc gọi method hoặc function thì không cần.
...
Tất cả những file PHP kết thúc ở cuối với một dòng trống.
...
Chỉ có một từ khóa `use` ở một dòng khai báo.
...
Mở ngoặc nhọn của class phải nằm ở dòng tiếp theo, và đóng ngoặc nhọn tại dòng cuối phía sau nội dung.
```php
<?php
namespace Vendor\Package;
...
Nếu một class có nhiều `implements` thì chúng ta cần phải chia theo nhiều dòng. Với interface đầu tiên nằm ở dòng tiếp theo, và mỗi interface nằm trên một dòng. 
```php
<?php
namespace Vendor\Package;
...
#### 4.2. Properties
Phạm vi cần được khai báo ở toàn bộ các property.

Từ khóa `var` thì không được dùng để khai báo property.
...
#### 4.3 Methods
Phạm vi cần được khai báo ở toàn bộ các method.

Tên method không nên bắt đầu bằng một gạch dưới để thể hiện phạm vi `protected` hay `private`.

Phía sau tên method không được có space nào cả. Mở ngoặc nhọn phải nằm ở dòng tiếp theo và đóng ở dòng kết tiếp của nội dung.
...
#### 4.4 Method argument
Trong danh sách argument, không được có whitespace trước dấu phẩy, và phải có một whitepsace sau dấu phẩy.
...
Danh sách argument có thể chia làm nhiều dòng, với argument đầu tiên nằm ở dòng tiếp theo, mỗi dòng là một argument.

Và khi chia như vậy, thì đóng ngoặc đơn và mở ngoặc nhọn cũng nằm ở một dòng riêng biệt.
```php
<?php
namespace Vendor\Package;
...
Từ khóa `abstract` và `final` phải nằm trước phạm vi, và `static` thì nằm sau phạm vi.
```php
<?php
namespace Vendor\Package;
...
Khi gọi method hoặc function, không có whitespace giữa tên và mở ngoặc đơn, phía sau mở ngoặc đơn và phía trước đóng ngoặc đơn.

Trong danh sách argument thì không có whitespace phía trước dấu phẩy, và có một whitespace phía sau dấu phẩy.
```php
<?php
bar();
$foo->bar($arg1);
...
Danh sách argument có thể chia làm nhiều dòng, mỗi mỗi dòng là một argument. Đóng ngoặc nhọn sẽ nằm ở một dòng riêng biệt.
```php
<?php
$foo->bar(
    $longArgument,
...
* Không có whitespace nào trước và sau đóng mở ngoặc đơn
